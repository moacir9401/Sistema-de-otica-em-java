...
// Métodos
	...
	public static boolean validarData(String wdata) {
		...
		if (auxData.length != 3)
			return false;
		if (auxData[0].equals("") || auxData[1].equals("") || auxData[2].equals(""))
			return false;
		// Valida mes
		...
		// Valida ano
		if (!auxData[2].matches("[0-9]*") || auxData[2].trim().length() < 2 || auxData[2].trim().length() == 3
				|| auxData[2].trim().length() > 4)
			return false;
	 ...
	 * @param dt    referência a um objeto da classe GregorianCalendar para receber
	 *              a data válida
	 * @return true para data válida ou false para data inválida
	 */
	 ...
	 * 
	 * @param wdata - String com a data
	 * @param dt    referência a um objeto da classe java.sql.Date para receber a
	 *              data válida
	 * @return true para data válida ou false para data inválida
	 */
	...
	public static boolean validarHora(String whora) {
		...
		if (hora1.length != 3)
			return false;
		if (hora1[0].equals("") || hora1[1].equals("") || hora1[2].equals(""))
			return false;
	 ...
	 * 
	 * @param wdata - String com a data no formato h:m:s ou hh:mm:ss
	 * @param dt    - Objeto GregorianCalendar dt para atualizar a hora na data
	 *              corrente
	 * @return true para horário válido ou false para horário inválido
	 */
	 ...
	 * 
	 * @param wdataHora - String com a data no formato d/M/yy h:m:s ou d/M/yyyy
	 *                  hh:mm:ss
	 * @return true para horário válido ou false para horário inválido
	 */
	public static boolean validarDataHora(String dataHora) {
		String wdataHora = padronizarTexto(dataHora);
		if (wdataHora.indexOf(" ") == -1)
			return false;
		String[] arrDataHora = wdataHora.split(" ");
		if (!validarData(arrDataHora[0]))
			return false;
		if (!validarHora(arrDataHora[1]))
			return false;
		return true;
	}
	...
	public static boolean validarCPF(String wcpf) {
		...
		wcpf = RetiraSimbolo(wcpf);
		
		if (wcpf.trim().equals("") || !wcpf.matches("[0-9]*") || wcpf.length() != 11)
			return false;
	...
	public static boolean validarCNPJ(String wcnpj) {
		if (wcnpj.trim().equals("") || !wcnpj.matches("[0-9]*") || wcnpj.length() != 14)
			return false;
	...
	public static boolean validarPIS(String wpis) {
		...
		if (wpis.trim().equals("") || !wpis.matches("[0-9]*") || wpis.length() > 11)
			return false;
		String aux = wpis;
		int soma = 0;
		for (int j = 0; j < 11 - wpis.length(); j++)
			aux = "0" + aux; // completar com zeros a esquerda
		for (int i = 0; i < aux.length() - 1; i++)
			soma += Integer.parseInt(aux.substring(i, i + 1)) * tab[i];
	...
	public static boolean validarEmail(String email) {
		int n = 0;
		if ((email.substring(0, 1).equals("@")) || (email.substring(email.length() - 1, email.length()).equals("@")))
			return false;
		else {
	...
	public static String formatarTelefone(String telefone) {
		try {
			...
			telefone = RetiraSimbolo(telefone);
	...
	public static String RetirarReal(String texto) {
		try {
	...
	public static String RetiraSimbolo(String texto) {
	 ...
	 * @param s        - String s
	 * @return String com espaços a esquerda e com o tamanho especificado por
	 *         tamCampo
	 */
	public static String espacosLeft(int tamCampo, String s) {
		String resp = s;
		for (int i = 0; i < tamCampo - s.length(); i++)
			resp = " " + resp;
		return resp;
	}

	/**
	 * Método subtrairDatas - Mostrar como achar a diferença entre duas datas em
	 * dias
	 * 
	...
	/**
	 * Método subtrairHoras - Mostrar como achar a diferença entre duas datas /
	 * horas
	 * 
	 * @param dt1         - data/hora de inicio do intervalo
	 * @param dt2         - data/hora final do intervalo
	 * @param tipoRetorno - 1: retorna horas, 2: retorna milisegundos, 3: retorna
	 *                    minutos, 4: retorna segundos
	...
	public static String formatarMilisegundos(long miliseg, int retorno) {
		...
		if (retorno == LtpLib.RETORNAR_SEGUNDOS) {
			return new DecimalFormat("#,###,#00").format(horas) + ":" + new DecimalFormat("00").format(minutos) + ":"
	...
	public static String valorExtenso(double valor) {
		...
		// Variaveis
		String[] vetor = { "", "Um", "Dois", "Tres", "Quatro", "Cinco", "Seis", "Sete", "Oito", "Nove", "Dez", "Onze",
				"Doze", "Treze", "Quatroze", "Quinze", "Dezesseis", "Dezessete", "Dezoito", "Dezenove", "", "", "Vinte",
				"Trinta", "Quarenta", "Cinquenta", "Sessenta", "Setenta", "Oitenta", "Noventa", "", "Cem", "Duzentos",
				"Trezentos", "Quatrocentos", "Quinhentos", "Seiscentos", "Setecentos", "Oitocentos", "Novecentos" };

		String[] vNomClasse = { "", "Real", "Reais", "Mil", "Mil", "Milhão", "Milhões", "Bilhão", "Bilhões", "Centavo",
				"Centavos" };
		...
		if (vDec) {
			...
			if (vAlg[2] > 1) {
				...
				if (vAlg[1] != 0)
					vExtenCla[5] = vExtenCla[5] + " e " + vetor[vAlg[1]];
			} else {
				if (vAlg[2] == 1)
					vExtenCla[5] = vExtenCla[5] + vetor[vAlg[1] + 10];
				else if (vAlg[1] != 0)
					vExtenCla[5] = vExtenCla[5] + vetor[vAlg[1]];
			}
			if (!vExtenCla[5].equals("Um"))
				vExtenCla[5] = vExtenCla[5] + " " + vNomClasse[2 * 5];
			else
				vExtenCla[5] = vExtenCla[5] + " " + vNomClasse[(2 * 5) - 1];
		}
//		 ----------------------------------------------------------;
		...
		while (aux2 <= aux) {
			...
			if (vAlg[3] != 0) {
				if ((vAlg[3] == 1) && ((vAlg[2] != 0) || (vAlg[1] != 0)))
					vExtenCla[aux2] = "Cento e ";
				else {
					vExtenCla[aux2] = vetor[vAlg[3] + 30];
					if (vAlg[2] != 0 || vAlg[1] != 0)
						vExtenCla[aux2] = vExtenCla[aux2] + " e ";
				}
			} else {
				vExtenCla[aux2] = "";
			}
			...
			if (vAlg[2] > 1) {
				...
				if (vAlg[1] != 0)
					vExtenCla[aux2] = vExtenCla[aux2] + " e " + vetor[vAlg[1]];
			} else {
				if (vAlg[2] == 1)
					vExtenCla[aux2] = vExtenCla[aux2] + vetor[vAlg[1] + 10];
				else if (vAlg[1] != 0)
					vExtenCla[aux2] = vExtenCla[aux2] + vetor[vAlg[1]];
			}
			if (!vExtenCla[aux2].equals("Um"))
				vExtenCla[aux2] = vExtenCla[aux2] + " " + vNomClasse[2 * aux2];
			else
				vExtenCla[aux2] = vExtenCla[aux2] + " " + vNomClasse[2 * aux2 - 1];

			aux2 = aux2 + 1;
		} // Final Loop

		if (aux2 > 1)
			vExtenso = vExtenCla[aux2 - 1];
		...
		while (aux2 > 0) {
			...
			if (aux2 == 1) {
				if (vExtenCla[aux2].trim().toUpperCase().substring(0, 3).equals("REA"))
					...
					vExtenso = vExtenso + " e " + vExtenCla[aux2];
			} else
				vExtenso = vExtenso + ", " + vExtenCla[aux2];
		...
		if (vDec)
			if (vExtenso.length() > 0)
				vExtenso = vExtenso + " e " + vExtenCla[5];
			else
				vExtenso = vExtenso + vExtenCla[5];
	...
	/**
	 * Método lerArquivoObjetos - Ler a lista de objetos do arquivo de objetos, se
	 * ele existir
	 * 
	 * @param nomeArquivo - representa o nome do arquivo em que está armazenado os
	 *                    objetos da lista
	 * @return Vector - referência para a lista de objetos controlados pela classe
	 *         ArrayList
	...
	/**
	 * Método lerArquivoObjetosArray - Ler a lista de objetos do arquivo de objetos,
	 * se ele existir
	 * 
	 * @param nomeArquivo - representa o nome do arquivo em que está armazenado os
	 *                    objetos da lista
	 * @return ArrayList - referência para a lista de objetos controlados pela
	 *         classe ArrayList
	 * @throws Exception - Exceção de java.io
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Método gravarArquivoObjetos - Grava a listas de objetos em um arquivo de
	 * objetos
	 * 
	 * @param nomeArquivo  - representa o nome do arquivo em que será gravado os
	 *                     objetos da lista
	 * @param listaObjetos - referência para os objetos da lista controlados pela
	 *                     classe Vector
	 * @throws Exception - Exceção de java.io
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Método gravarArquivoObjetosArray - Grava a listas de objetos em um arquivo de
	 * objetos
	 * 
	 * @param nomeArquivo  - representa o nome do arquivo em que será gravado os
	 *                     objetos da lista
